...
int sensorMsgsEncodingToOpenCVType(const std::string &encoding)
{
  ...
  RCLCPP_ERROR(
      rclcpp::get_logger("vikit_img_type_conversion"),
      "[sensorMsgsEncodingToOpenCVType] Invalid sensor_msgs encoding was passed");
  return -1;
}
...
std::string openCVTypeToSensorMsgsEncoding(const int opencv_type)
{
  ...
  RCLCPP_ERROR(
      rclcpp::get_logger("vikit_img_type_conversion"),
      "[openCVTypeToSensorMsgsEncoding] Invalid OpenCV type was passed");
  return "";
}
