...
void SchurVINS::Prediction(double _dt, const Eigen::Vector3d& _acc, const Eigen::Vector3d& _gyr) {
    CHECK(_dt >= 0) << "dt: " << _dt;
    constexpr double kMaxPredictionDt = 0.01;
    if (_dt > kMaxPredictionDt) {
        const int n_steps = static_cast<int>(std::ceil(_dt / kMaxPredictionDt));
        const double dt_step = _dt / n_steps;
        for (int i = 0; i < n_steps; ++i) {
            Prediction(dt_step, _acc, _gyr);
        }
        return;
    }
