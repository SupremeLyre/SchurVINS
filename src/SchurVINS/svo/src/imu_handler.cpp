...
{
  const double target_timestamp_sec =
      img_timestamp_sec - this->imu_calib_.delay_imu_cam;
  const auto wait_start = std::chrono::steady_clock::now();
  while (this->getLatestTimestamp() < target_timestamp_sec)
  {
    const std::chrono::duration<double> elapsed =
        std::chrono::steady_clock::now() - wait_start;
    if (elapsed.count() > timeout_sec)
    {
      LOG(ERROR) << "Did not get IMU measurements up to "
                 << target_timestamp_sec << " after " << timeout_sec
                 << " seconds. Latest IMU timestamp: "
